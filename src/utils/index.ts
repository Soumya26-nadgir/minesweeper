...
export const getTimeDiff = (timeNow: Date | null, timeStarted: Date | null) => {
  if (timeNow === null || timeStarted === null) return "000";

  const diffInSeconds = Math.floor(
    (timeNow.getTime() - timeStarted.getTime()) / 1000,
  );

  // Format as 3 digits for classic minesweeper feel, or standard MM:SS
  // User asked for "dynamic", normally minesweeper is simply seconds count "001", "002"
  // But let's stick to the previous attempt which seemed to want time structure.
  // Actually, standard modern web minesweeper often uses MM:SS or simply seconds.
  // Given the previous code tried MM:SS, let's Stick to MM:SS but correct logic.
